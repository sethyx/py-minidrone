...
def get_current_setting(pattern):
    return hex2vals(''.join(pattern.split(' ')[6:9]))
...
#     return hex2vals(''.join(pattern.split(' ')[10:13]))
#
# def get_max_setting(pattern):
#     return hex2vals(''.join(pattern.split(' ')[14:17]))

def get_misc(pattern):
    return True if pattern.split(' ')[-1] == '01' else False
...
def onoff(bool):
    return C_ON if bool else C_OFF

def get_pretty(config, setting):
    if setting == S_WHEELS || setting == S_CUTOUT:
        return onoff(config[setting])
    else:
        return config[setting] + ' ' + UNITS[setting]